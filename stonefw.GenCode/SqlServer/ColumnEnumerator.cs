...
namespace stonefw.CodeGenerate.SqlServer
{
    public static class ColumnEnumerator
    {
        ...
        {
            const string strSql = "select c.[name] from [sys].[tables] as t,[sys].[columns] as c where t.[name]=@TheTable and t.[object_id]=c.[object_id] and c.[is_identity]='1'";
            try
            {
        ...
        {
            const string strSql = "select c.[name] from [sys].[tables] as t,[sys].[columns] as c,[sys].[indexes] as i,[sys].[index_columns] as ic where t.[name]=@TheTable and i.[is_primary_key]='1' and t.[object_id]=c.[object_id] and t.[object_id]=i.[object_id] and t.[object_id]=ic.[object_id] and c.[column_id]=ic.[column_id] and i.[index_id]=ic.[index_id]";
