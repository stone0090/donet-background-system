...
namespace stonefw.CodeGenerate
{
    public partial class MainForm : Form
    {
        ...
        private void CreateCodeOfBiz(string folder)
        {
            ...
            {
                var type = GetObjectTypeOfDBColumnForCode(columns.Where(m => m.ColumnName == primaryKeyColumns[i]).ToList()[0].ColumnType);
            ...
            for (int i = 0; i < primaryKeyColumns.Length; i++)
            {
                ...
                strQueryArg2 += string.Format("n.{0} == {1}&&", primaryKeyColumns[i], FirstCharLower(primaryKeyColumns[i]));
            }
        ...
        private void CreateCodeOfUi(string folder)
        {
            ...
            string fileOfDetailAspxDesignerCs = folder + ns + ".web\\" + this.txtEntityName.Text + "Detail.aspx.designer.cs";
        ...
        private void CreateCodeOfListAspx(string fileName)
        {
            ...
            codeBuffer.AppendLine("<asp:LinkButton runat=\"server\" ID=\"btnQuery\" Text=\"查询\" class=\"easyui-linkbutton\" data-options=\"iconCls:'icon-search'\" OnClick=\"btnQuery_Click\" ></asp:LinkButton>");

            //新增
            string strAddArg = primaryKeyColumns.Aggregate(string.Empty, (current, primaryKey) => current + primaryKey.Replace("_", "").ToLower() + "=-1&");
            strAddArg = strAddArg.Substring(0, strAddArg.Length - 1);
            codeBuffer.Append("<asp:LinkButton runat=\"server\" ID=\"btnAddNew\" Text=\"新增\" class=\"easyui-linkbutton\" data-options=\"iconCls:'icon-add'\" OnClientClick=\"return showDialog('新增', '").Append(n).Append("Detail.aspx?").Append(strAddArg).AppendLine("',this);\">新增</asp:LinkButton>");

            codeBuffer.AppendLine("</td></tr></table></div>");
            codeBuffer.Append("<asp:GridView ID=\"gv").Append(n).Append("\" runat=\"server\" AutoGenerateColumns=\"False\" AllowPaging=\"True\" Width=\"100%\" CssClass=\"gridview\"");
            codeBuffer.Append("OnRowCommand=\"gv").Append(n).Append("_RowCommand\" OnPageIndexChanged=\"gv").Append(n).Append("_PageIndexChanged\" OnPageIndexChanging=\"gv").Append(n).Append("_PageIndexChanging\" >");
            ...
            string strDeleteArg = primaryKeyColumns.Aggregate(string.Empty, (current, primaryKey) => current + string.Format("Eval(\"{0}\")+\"|\"+", primaryKey));
            strDeleteArg = strDeleteArg.Substring(0, strDeleteArg.Length - 5);
            codeBuffer.Append("<asp:LinkButton class=\"easyui-linkbutton\" runat=\"server\" CommandName=\"Row_Delete\" CommandArgument='<%# ").Append(strDeleteArg).AppendLine(" %>' OnClientClick=\"return deleteWarning(this);\">删除</asp:LinkButton>");
            codeBuffer.AppendLine("</ItemTemplate>");
            ...
            codeBuffer.AppendLine("<ItemTemplate>");
            string strEditArg = primaryKeyColumns.Aggregate(string.Empty, (current, primaryKey) => current + string.Format(primaryKey.Replace("_", "").ToLower() + "=\"+Eval(\"{0}\")+\"&", primaryKey));
            strEditArg = strEditArg.Substring(0, strEditArg.Length - 1);
            codeBuffer.Append("<a href=\"#\" class=\"easyui-linkbutton\" onclick='<%# \"showDialog(\\\"修改\\\", \\\"").Append(n).Append("Detail.aspx?").Append(strEditArg).AppendLine("\\\");\" %>'>修改</a>");
            codeBuffer.AppendLine("</ItemTemplate>");
            codeBuffer.AppendLine("</asp:TemplateField>");

            foreach (SqlServerColumn column in columns)
            {
                codeBuffer.AppendLine(string.Format("<asp:BoundField DataField=\"{0}\" HeaderText=\"{0}\" />", column.ColumnName));
            }
        ...
        private void CreateCodeOfListAspxCs(string fileName)
        {
            ...
            codeBuffer.AppendLine("}");
            codeBuffer.Append("protected void gv").Append(n).AppendLine("_RowCommand(object sender, GridViewCommandEventArgs e){");
            ...
            codeBuffer.AppendLine("BindData();}}");
            codeBuffer.Append("protected void gv").Append(n).AppendLine("_PageIndexChanged(object sender, EventArgs e){BindData();");
            codeBuffer.AppendLine("}");
            codeBuffer.Append("protected void gv").Append(n).AppendLine("_PageIndexChanging(object sender, GridViewPageEventArgs e)");
            codeBuffer.Append("{this.gv").Append(n).AppendLine(".PageIndex = e.NewPageIndex;");
            codeBuffer.AppendLine("}");
            codeBuffer.AppendLine("");
            codeBuffer.AppendLine("private void BindData()");
            codeBuffer.AppendLine("{");
            codeBuffer.Append("gv").Append(n).AppendLine(".PageSize = int.Parse(base.SysGlobalSetting.GridViewPageSize);");
        ...
        private void CreateCodeOfDetailAspx(string fileName)
        {
            ...
            {
                codeBuffer.AppendLine(string.Format("<asp:HiddenField runat=\"server\" ID=\"{0}\" />", FirstCharLower("hd" + column)));
            }
            ...
            foreach (var column in columns)
            {
                ...
                {
                    case "string":
                        codeBuffer.AppendLine(string.Format("<td><asp:TextBox ID=\"{0}\" runat=\"server\" MaxLength=\"{1}\" {2}  class=\"easyui-textbox\" data-options=\"required:true\"></asp:TextBox></td>",
                            FirstCharLower("txt" + column.ColumnName), column.ColumnSize / 2, isReadonly));
                        break;
                    case "DateTime":
                        codeBuffer.AppendLine(string.Format("<td><asp:TextBox ID=\"{0}\" runat=\"server\" class=\"easyui-textbox\" data-options=\"required:true\"></asp:TextBox></td>",
                    ...
                    case "double":
                    case "byte":
                        codeBuffer.AppendLine(string.Format("<td><asp:TextBox ID=\"{0}\" runat=\"server\" {1} class=\"easyui-textbox\" data-options=\"required:true\"></asp:TextBox></td>",
                            FirstCharLower("txt" + column.ColumnName), isReadonly));
                        break;
                    case "bool":
                        codeBuffer.AppendLine("<td>");
                        codeBuffer.AppendLine(string.Format("<asp:CheckBox runat=\"server\" ID=\"{0}\" Text=\"{1}\" />", FirstCharLower("cb" + column.ColumnName), column.ColumnName));
                        codeBuffer.AppendLine("</td>");
                        break;
                }
                codeBuffer.AppendLine("</tr>");
            ...
            codeBuffer.AppendLine("<asp:LinkButton ID=\"btnSave\" runat=\"server\" class=\"easyui-linkbutton\" data-options=\"iconCls:'icon-ok'\" OnClientClick=\"return saveForm(this);\" OnClick=\"btnSave_Click\">保存</asp:LinkButton>");
            codeBuffer.AppendLine("<a class=\"easyui-linkbutton\" data-options=\"iconCls:'icon-cancel'\" href=\"#\" onclick=\"window.parent.closeDialog();\">取消</a>");
            codeBuffer.AppendLine("</div>");
            codeBuffer.AppendLine("</div>");
        ...
        private void CreateCodeOfDetailAspxCs(string fileName)
        {
            ...
            //新增
            string strAddArg = primaryKeyColumns.Aggregate(string.Empty, (current, primaryKey) => current + string.Format("this.{0}.Value == \"-1\"&&", FirstCharLower("hd" + primaryKey)));
            ...
            codeBuffer.AppendLine("}catch (Exception ex){this.lMessage.Text = string.Format(\"保存失败，原因：{0}\", ex.Message);}");
            codeBuffer.AppendLine("}");
            codeBuffer.AppendLine("");
            ...
            foreach (string column in primaryKeyColumns)
            {
                codeBuffer.AppendLine(string.Format("this.{0}.Value = Request[\"{1}\"];", FirstCharLower("hd" + column), column.Replace("_", "").ToLower()));
            }
            ...
            for (int i = 0; i < primaryKeyColumns.Length; i++)
            {
                var type = GetObjectTypeOfDBColumnForCode(columns.Where(m => m.ColumnName == primaryKeyColumns[i]).ToList()[0].ColumnType);
                if (type == "int")
                    strQeruyArg += string.Format("{0}.Parse(this.{1}.Value),", type, FirstCharLower("hd" + primaryKeyColumns[i]));
                else
            ...
            foreach (SqlServerColumn column in columns)
            {
                ...
                codeBuffer.AppendLine(type == "bool"
                    ? string.Format("this.{0}.Checked = (bool)entity.{1};", FirstCharLower("cb" + column.ColumnName), column.ColumnName)
                    : string.Format("this.{0}.Text = entity.{1}.ToString();", FirstCharLower("txt" + column.ColumnName), column.ColumnName));
            }

            codeBuffer.AppendLine("}}catch (Exception ex){this.lMessage.Text = string.Format(\"数据加载失败，原因：{0}\", ex.Message);}");
            codeBuffer.AppendLine("}");
            ...
            foreach (SqlServerColumn column in columns)
            {
                ...
                {
                    case "string":
                        codeBuffer.AppendLine(string.Format("entity.{0} = this.{1}.Text;", column.ColumnName, FirstCharLower("txt" + column.ColumnName)));
                        break;
                    case "bool":
                        codeBuffer.AppendLine(string.Format("entity.{0} = this.{1}.Checked;", column.ColumnName, FirstCharLower("cb" + column.ColumnName)));
                        break;
                    default:
                        codeBuffer.AppendLine(string.Format("entity.{0} = {1}.Parse(this.{2}.Text);", column.ColumnName, type, FirstCharLower("txt" + column.ColumnName)));
                        break;
                }
            }
