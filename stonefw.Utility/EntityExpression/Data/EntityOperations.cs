...
using stonefw.Utility.EntityExpression.Utilitys;
using stonefw.Utility.EntityExpression.Entity;
using stonefw.Utility.EntityExpression.GenSQL;

namespace stonefw.Utility.EntityExpression.Data
{
    /// <summary>
    /// 实体类的操作
    /// </summary>
    public static class EntityOperations
    {
        ...
        public static int DeleteEntity<T>(T entity, Database db)
        {
            //获取主键相关的字段
            Type entityType = typeof(T);
            List<string> dbPrimaryKeys = DbTableMapping.GetEntityFieldPrimaryKey(entityType);
        ...
        public static T ReadEntity<T>(GenericWhereEntity<T> whereEntity, Database db) where T : class, new()
        {
            ...
            List<PropertyInfo> entityPropertyInfos = new List<PropertyInfo>();
            BuildDbColumnSelect.FillEntityMappings(whereEntity.EntityType, dbColumnNames, entityPropertyInfos);
            ...
            Database db) where MainTable : class, new()
        {
            ...
            List<string> dbColumnNames = new List<string>();
            List<PropertyInfo> entityPropertyInfos = new List<PropertyInfo>();
            BuildDbColumnSelect.FillEntityMappings(joinEntity.MainEntity.EntityType, dbColumnNames, entityPropertyInfos);
            ...
            Database db) where SubTable : class, new()
        {
            ...
            List<string> dbColumnNames = new List<string>();
            List<PropertyInfo> entityPropertyInfos = new List<PropertyInfo>();
            BuildDbColumnSelect.FillEntityMappings(joinEntity.EntityToJoin.EntityType, dbColumnNames, entityPropertyInfos);
            ...
            params int[] maxRowCounts) where DetailsTable : class, new()
        {
            ...
            List<string> dbColumnNames = new List<string>();
            List<PropertyInfo> entityPropertyInfos = new List<PropertyInfo>();
            BuildDbColumnSelect.FillEntityMappings(joinEntity1.EntityToJoin.EntityType, dbColumnNames, entityPropertyInfos);
            int rowCount = (maxRowCounts == null || maxRowCounts.Length < 1) ? 0 : maxRowCounts[0];
