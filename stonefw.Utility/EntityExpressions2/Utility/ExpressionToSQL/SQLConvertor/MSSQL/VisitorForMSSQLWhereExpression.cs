...
using stonefw.Utility.EntityExpressions2.Utility.ExpressionToSQL.SQLConvertor.MSSQL.MethodCalls;

namespace stonefw.Utility.EntityExpressions2.Utility.ExpressionToSQL.SQLConvertor.MSSQL
{
    /// <summary>
    /// 用于查询条件生成的访问其
    /// </summary>
    /// <typeparam name="W"></typeparam>
    internal class VisitorForMSSQLWhereExpression<W> : BaseVisitorForWhereExpression
    {
        /// <summary>
        /// 构造函数
        /// </summary>
        ...
        public VisitorForMSSQLWhereExpression(string tableAlias, GenericWhereEntity<W> theEntity)
        {
        ...
        {
            ExpressionPreTreatment evaluator = new ExpressionPreTreatment();
            Expression evaluatedExpression = evaluator.DoPreTreatment(expression);
        ...
        protected override Expression VisitBinary(BinaryExpression b)
        {
            ...
            if (isLeftMember)
            {
                m_TmpDBColumnName = leftMemberExp.GetDBColumnName(_TheWhereEntity.EntityType);
                m_TmpDBColumnType = leftMemberExp.GetDBColumnType(_TheWhereEntity.EntityType);
            ...
            {
                m_TmpDBColumnName = rightMemberExp.GetDBColumnName(_TheWhereEntity.EntityType);
                m_TmpDBColumnType = rightMemberExp.GetDBColumnType(_TheWhereEntity.EntityType);
            }
            this.Visit(b.Left);
            //两侧都是访问成员的话，也加载
        ...
        protected override Expression VisitMethodCall(MethodCallExpression m)
        {
            ...
            {
                ResolveMethodCall(_TheWhereEntity.EntityType, m, _TableAlias, this.m_conditionParts, this.m_ParameterNames, this.m_DbTypes, this.m_arguments);
            }
            else
            {
        ...
        private void ResolveMethodCall(Type theEntityType, MethodCallExpression m, string tableAlias, Stack<string> colConditionParts, List<string> colParameterNames, List<DbType> colDbTypes, List<object> colArguments)
        {
            if (m.Object is MemberExpression)
            {
                ...
                {
                    MSSQLStringMethodCallVisitor.Visit(theEntityType, m, tableAlias, colConditionParts, colParameterNames, colDbTypes, colArguments);
                }
                else
                {
