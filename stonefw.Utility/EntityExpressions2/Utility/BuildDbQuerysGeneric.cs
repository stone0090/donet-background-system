...
using System.Linq;
using System.Text;
using stonefw.Utility.EntityExpressions2.Utility.ExpressionVisitors;
    ...
    public static class BuildDbQuerysGeneric
    {
        ...
        public static string BuildCondition<WA>(GenericWhereEntity<WA> theWhereEntity)
        {
            ...
            if (theWhereEntity.WhereExpressions.Count > 0)
            {
                ...
                {
                    ConditionBuilderGeneric<WA> conditionBuilder = new ConditionBuilderGeneric<WA>((theWhereEntity.DisableTableAlias ? dbTableName : theWhereEntity.TableNameDeclare), theWhereEntity);
        ...
        private static void BuildConditionForJoin<WA>(GenericWhereEntity<WA> theWhereEntity)
        {
            ...
            if (theWhereEntity.WhereExpressions.Count > 0)
            {
                ...
                for (int i = 0; i < theWhereEntity.WhereExpressions.Count; i++)
                {
                    ConditionBuilderGeneric<WA> conditionBuilder = new ConditionBuilderGeneric<WA>(theWhereEntity.TableNameDeclare, theWhereEntity);
                    conditionBuilder.Build(theWhereEntity.WhereExpressions[i]);
                    if (i > 0)
                    {
                        tsqlBuffer.Append(" AND ");
