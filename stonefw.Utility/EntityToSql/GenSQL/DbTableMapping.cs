...
namespace stonefw.Utility.EntityToSql.GenSQL
{
    internal static class DbTableMapping
    {
        ...
        /// 缓存
        /// </summary>
        private static List<DbTableMappingEntity> _dbTableMappingEntityBuffer = null;

        static DbTableMapping()
        {
            _dbTableMappingEntityBuffer = new List<DbTableMappingEntity>(1024);
        }

        public static DbTableMappingEntity GetDbTableMappingEntity(Type entityType)
        {
            lock (_dbTableMappingEntityBuffer)
            {
                var typeName = entityType.FullName;
                var buffer = _dbTableMappingEntityBuffer.FirstOrDefault(n => n.EntityTypeName == typeName);
                if (buffer != null)
                    return buffer;
                ...
                buffer = new DbTableMappingEntity()
                {
                ...
                _dbTableMappingEntityBuffer.Add(buffer);
