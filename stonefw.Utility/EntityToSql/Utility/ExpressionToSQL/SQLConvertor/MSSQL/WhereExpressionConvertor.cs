    ...
    internal class WhereExpressionConvertor : IWhereExpressionConvertor
    {
        ...
        public string BuildCondition<TA>(GenericWhereEntity<TA> theWhereEntity)
        {
            ...
            if (theWhereEntity.WhereExpressions.Count > 0)
            {
                ...
                {
                    VisitorForMSSQLWhereExpression<TA> conditionBuilder = new VisitorForMSSQLWhereExpression<TA>((theWhereEntity.DisableTableAlias ? dbTableName : theWhereEntity.TableName), theWhereEntity);
        ...
        private static void BuildConditionForJoin<TA>(GenericWhereEntity<TA> theWhereEntity)
        {
            ...
            if (theWhereEntity.WhereExpressions.Count > 0)
            {
                ...
                for (int i = 0; i < theWhereEntity.WhereExpressions.Count; i++)
                {
                    VisitorForMSSQLWhereExpression<TA> conditionBuilder = new VisitorForMSSQLWhereExpression<TA>(theWhereEntity.TableName, theWhereEntity);
                    conditionBuilder.Build(theWhereEntity.WhereExpressions[i]);
                    if (i > 0)
                    {
                        tsqlBuffer.Append(" AND ");
