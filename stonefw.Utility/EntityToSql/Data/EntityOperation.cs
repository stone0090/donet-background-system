...
namespace stonefw.Utility.EntityToSql.Data
{
    ...
    public static class EntityOperation
    {
        ...
        public static void InsertEntity<T>(T entity, Database db = null)
        {
            ...
            using (DbCommand cmd = SqlCreator.CreateInsertCommand(db, entity))
            {
                ...
                if (rowCount <= 0) throw new EntityToSqlException("新增记录失败(DB)！");
            }
        }
        ...
        public static object InsertEntityWithIdentity<T>(T entity, Database db = null)
        {
            if (db == null) db = DatabaseFactory.CreateDatabase();
        ...
        public static int DeleteEntity<T>(Expression<Func<T, bool>> conditionExpression, Database db = null)
        {
            if (conditionExpression == null)
                throw new EntityToSqlException("删除记录时，必须指定Where条件，否则将导致整个表的数据被删除！");
        ...
        /// <param name="db">数据库连接</param>
        /// <returns>受影响的行数</returns>
        public static int DeleteEntity<T>(GenericWhereEntity<T> whereEntity, Database db = null)
        {
            if (db == null) db = DatabaseFactory.CreateDatabase();
        ...
        /// <param name="db">数据库连接</param>
        /// <returns>受影响的行数</returns>
        public static int DeleteEntity<T>(T entity, Database db = null)
        {
            //获取主键相关的字段
            Type entityType = typeof(T);
            if (!DbTableMapping.HasPrimaryKey(entityType))
                throw new EntityToSqlException(string.Format("实体类{0}未设置主键字段！", entityType.FullName));
        ...
        /// <param name="entity">实体类实例</param>
        /// <param name="db">数据库连接</param>
        public static void UpdateEntity<T>(T entity, Database db = null)
        {
            //检索数据库相关的字段和属性
            Type entityType = typeof(T);
            ...
            if (!EntityInstanceTool.HasPrimaryKeyValue(entity))
                throw new EntityToSqlException(string.Format("未给实体类{0}实例的主键字段赋值！", entityType.FullName));

            if (db == null) db = DatabaseFactory.CreateDatabase();
            using (DbCommand cmd = SqlCreator.CreateUpdateCommand(db, entity))
            {
                int rowCount = db.ExecuteNonQuery(cmd);
                if (rowCount <= 0) throw new EntityToSqlException("修改记录失败(DB)！");
            }
        }
        ...
        /// <param name="entity">实体类实例</param>
        /// <param name="db">数据库连接</param>
        public static void UpdateEntity<T>(Expression<Func<T, bool>> conditionExpression, T entity, Database db = null)
        {
            if (conditionExpression == null)
                throw new EntityToSqlException("修改记录时，必须指定Where条件，否则将导致整个表的数据被修改！");
        ...
        /// <param name="entity">实体类实例</param>
        /// <param name="db">数据库连接</param>
        public static void UpdateEntity<T>(GenericWhereEntity<T> whereEntity, T entity, Database db = null)
        {
            if (db == null) db = DatabaseFactory.CreateDatabase();
        ...
        /// <param name="db">数据库连接</param>
        /// <returns></returns>
        public static T ReadEntity<T>(Expression<Func<T, bool>> conditionExpression, Database db = null) where T : class, new()
        {
            GenericWhereEntity<T> whereEntity = new GenericWhereEntity<T>();
            if (conditionExpression != null)
                whereEntity.Where(conditionExpression);
        ...
        public static T ReadEntity<T>(GenericWhereEntity<T> whereEntity, Database db = null) where T : class, new()
        {
        ...
        public static List<T> ReadEntityList<T>(Expression<Func<T, bool>> conditionExpression, Database db = null, params int[] maxRowCounts) where T : class, new()
        {
            GenericWhereEntity<T> whereEntity = new GenericWhereEntity<T>();
            if (conditionExpression != null)
                whereEntity.Where(conditionExpression);
        ...
        /// <param name="maxRowCounts">读取的记录数量</param>
        /// <returns>实体类的列表</returns>
        public static List<T> ReadEntityList<T>(GenericWhereEntity<T> whereEntity, Database db = null, params int[] maxRowCounts) where T : class, new()
        {
            ...
            using (var cmd = db.GetSqlStringCommand(selectSql + whereSql))
            {
                ...
                using (var reader = db.ExecuteReader(cmd))
                {
                    ...
                    while (reader.Read())
                    {
                        var entity = EntityInstanceTool.FillOneEntity<T>(reader);
                        entitys.Add(entity);
                    }
                    return entitys;
                }
        ...
        {
            MemberExpression m = keySelector.Body as MemberExpression;
            List<string> dbColumnNames = new List<string>(1) { DbTableMapping.GetDbColumnName(typeof(T), m.Member.Name) };
            string whereSql = BuildDbQuerysGeneric.BuildCondition(whereEntity);
